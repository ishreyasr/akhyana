...
function RegisterForm() {
    ...
    const [step, setStep] = useState(1); // 1 personal, 2 vehicle (google optional on vehicle page)
    ...
    const finish = async () => {
        ...
        try {
            ...
            // Google linking is optional - continue with or without it
                    ...
                    {step === 2 && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-xs text-muted-foreground">Link your Google account for easier sign-in. You can skip this and link it later.</p>
                                <Button variant={googleLinked ? 'secondary' : 'outline'} disabled={loading || googleLinked} onClick={handleGoogle} className="w-full flex items-center gap-2 justify-center">
                                    ...
                                    {googleLinked ? 'Google Linked ✓' : 'Link with Google'}
                                </Button>
                            </div>
                            <div className="flex justify-between pt-2">
                                <Button variant="outline" onClick={back}>Back</Button>
                                <Button onClick={finish} disabled={!canContinueVehicle || loading}>{loading ? 'Creating...' : 'Create Account'}</Button>
                            </div>
                        </div>
                    )}
